...
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS payment_hold_minutes INTEGER DEFAULT 30;

-- 1. Create the function to cancel unpaid bookings and queue notification
...
BEGIN
    FOR booking IN
        ...
        WHERE b.status = 'pending_deposit'
          AND b.created_at < NOW() - make_interval(mins => COALESCE(t.payment_hold_minutes, 30))
    LOOP
        -- Update the status to 'cancelled'
        UPDATE bookings SET status = 'cancelled' WHERE id = booking.booking_id;

        -- Queue the expriation notification
...
SELECT cron.schedule('expire_unpaid_bookings', '*/5 * * * *', 'SELECT expire_pending_bookings()');
