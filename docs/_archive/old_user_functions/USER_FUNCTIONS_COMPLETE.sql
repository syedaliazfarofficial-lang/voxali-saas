...
-- Create profile with email column
...
WHERE id = p_user_id;
RETURN json_build_object('success', true, 'message', 'Deleted');
EXCEPTION
WHEN OTHERS THEN RAISE EXCEPTION 'Error: %',
SQLERRM;
END;
...
IF v_user_id IS NOT NULL THEN -- Update or insert profile
...
SET email = 'owner@gmail.com',
    ...
    role = 'owner',
    can_login = true;
RAISE NOTICE 'Owner profile fixed!';
END IF;
END $$;
