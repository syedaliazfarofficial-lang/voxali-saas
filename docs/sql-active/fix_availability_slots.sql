-- FIX: get_available_slots — corrected column names
-- b.start_at → b.start_time, b.end_at → b.end_time
...
WHILE v_slot_start + (v_total_duration || ' minutes')::interval <= v_slot_end LOOP -- FIX: Use start_time/end_time (correct column names, not start_at/end_at)
SELECT COUNT(*) INTO v_conflict_count
FROM bookings b
WHERE b.stylist_id = v_staff.id
    AND b.status NOT IN ('cancelled', 'no_show')
    AND b.start_time < (
        (p_date + v_slot_start) AT TIME ZONE COALESCE(v_tenant.timezone, 'America/Chicago') + (v_total_duration || ' minutes')::interval
    )
    AND b.end_time > (
        (p_date + v_slot_start) AT TIME ZONE COALESCE(v_tenant.timezone, 'America/Chicago')
    );
