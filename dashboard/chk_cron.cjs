...
async function testQuery() {
    console.log("Checking PG Cron Logs...");
    const { data: logs, error } = await supabase.rpc('exec_sql', {
        query: `SELECT jobid, command, status, return_message, start_time FROM cron.job_run_details ORDER BY start_time DESC LIMIT 5;`
    });
