...
async function test() {
    const { data: staff, error: e1 } = await supabase.from('staff').select('*').eq('tenant_id', 'e3c9b32f-7f4c-44b9-b0b6-97b00b7c0288');
    console.log("Staff:\n", staff);
    
    const { data: timeoff, error: e2 } = await supabase.from('staff_timeoff').select('*').eq('tenant_id', 'e3c9b32f-7f4c-44b9-b0b6-97b00b7c0288');
    console.log("Timeoff:\n", timeoff);
}
test();
