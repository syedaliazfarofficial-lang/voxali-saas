...
export const StaffBoard: React.FC = () => {
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {/* Unified Performance Overview Card */}
            <div className="glass-panel p-6 border border-white/10 rounded-2xl bg-gradient-to-r from-white/[0.02] to-white/[0.01] shadow-2xl relative overflow-hidden group animate-in fade-in duration-500">
                {/* Background decorative glow */}
                <div className="absolute -right-20 -top-20 w-64 h-64 bg-luxe-gold/5 rounded-full blur-3xl pointer-events-none transition-all duration-700 group-hover:bg-luxe-gold/10"></div>
                
                <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 relative z-10">
                    
                    {/* Left: Active Staff Avatar Stack + Summary */}
                    <div className="flex items-center gap-4 w-full lg:w-auto">
                        ...
                        <div className="flex -space-x-3 overflow-hidden flex-shrink-0">
                                ...
                                <div key={s.id} className="relative inline-block">
                                    {s.photo_url ? (
                                        <img
                                            src={s.photo_url}
                                            alt={s.full_name}
                                            className="w-10 h-10 rounded-full object-cover border-2 border-[#0D0D0D] shadow-md ring-1 ring-white/10"
                                        />
                                    ) : (
                                        <div
                                            className="w-10 h-10 rounded-full flex items-center justify-center text-xs font-black border-2 border-[#0D0D0D] shadow-md ring-1 ring-white/10"
                                            style={{
                                                color: s.color,
                            ...
                            {activeStaff.length > 5 && (
                                <div className="w-10 h-10 rounded-full bg-white/5 border-2 border-[#0D0D0D] flex items-center justify-center text-xs font-bold text-white/60 shadow-md ring-1 ring-white/10">
                                    +{activeStaff.length - 5}
                                </div>
                            )}
                        </div>
                        ...
                        <div>
                            <p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/30">Active Stylists</p>
                            <p className="text-base font-black text-white">{activeStaff.length} Team Members</p>
                        </div>
                    </div>

                    {/* Divider for desktop */}
                    <div className="hidden lg:block h-10 w-[1px] bg-white/10 flex-shrink-0" />

                    {/* Middle Left: Total Bookings */}
                    <div className="flex items-center gap-3 w-full lg:w-auto justify-start">
                        <div className="p-2 bg-sky-500/10 rounded-xl text-sky-400 border border-sky-500/20 flex-shrink-0">
                            <BarChart3 className="w-4 h-4" />
                        </div>
                        <div>
                            <p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/30">Total Bookings</p>
                            <p className="text-sm font-bold text-white">{totalBookings} appointments</p>
                        </div>
                    </div>
                    ...
                    {/* Middle Right: Total Revenue */}
                    <div className="flex items-center gap-3 w-full lg:w-auto justify-start">
                        <div className="p-2 bg-emerald-500/10 rounded-xl text-emerald-400 border border-emerald-500/20 flex-shrink-0">
                            <TrendingUp className="w-4 h-4" />
                        </div>
                        <div>
                            <p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/30">Total Revenue</p>
                            <p className="text-sm font-bold text-emerald-400">${totalRev.toLocaleString()}</p>
                        </div>
                    </div>
                    ...
                    {/* Right: Top Performer (MVP) */}
                    {activeStaff.length > 0 && (
                        <div className="flex items-center gap-3 w-full lg:w-auto justify-start">
                            {(() => {
                                const topStylist = [...activeStaff].sort((a, b) => b.revenue - a.revenue)[0];
                                return (
                                    <>
                                        <div className="relative flex-shrink-0">
                                            {topStylist.photo_url ? (
                                                <img
                                                    src={topStylist.photo_url}
                                                    alt={topStylist.full_name}
                                                    className="w-10 h-10 rounded-full object-cover border-2 border-luxe-gold/50 shadow-lg"
                                                />
                                            ) : (
                                                <div
                                                    className="w-10 h-10 rounded-full flex items-center justify-center text-xs font-black border-2 border-luxe-gold/50 shadow-lg"
                                                    style={{
                                                        color: topStylist.color,
                                                        backgroundColor: `${topStylist.color}15`,
                                                    }}
                                                >
                                                    {topStylist.full_name.charAt(0)}
                                                </div>
                                            )}
                                            {/* MVP Badge */}
                                            <span className="absolute -top-1.5 -right-1.5 bg-luxe-gold text-luxe-obsidian text-[8px] font-black px-1.5 py-0.5 rounded-full shadow border border-[#0D0D0D]">
                                                MVP
                                            </span>
                                        </div>
                                        <div>
                                            <p className="text-[9px] font-black uppercase tracking-[0.2em] text-luxe-gold flex items-center gap-1">
                                                Top Performer <Trophy className="w-2.5 h-2.5" />
                                            </p>
                                            <p className="text-sm font-bold text-white">{topStylist.full_name}</p>
                                        </div>
                                    </>
                                );
                            })()}
